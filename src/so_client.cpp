...
struct spam_status
{
    ...
    static so_client::spam_status_t parse_so_answer(const std::string &_buffer, envelope_ptr _envelope)
    {
        ...
        while (ss.good())
        {
            ...
            if (strncmp (line.c_str(), "SODAEMON ", 9) == 0)
            {
                int answer_size = 0;
            ...
            else if (strncmp (line.c_str(), "SPAMSTR ", 8) == 0)
            {
                ...
                if (line.find("X-Spam-Flag: DLVR") != std::string::npos)
                {
        ...
        try
        {
            ...
            if (process_answer(response_stream))
            {
                ...
                if  (m_proto_state == STATE_AFTER_DOT)
                {
                    ...
                    unsigned int send_size = m_envelope_size >  K64 ? K64 : m_envelope_size;
...
void so_client::restart()
{
    ...
    server_parameters::remote_point info = (m_so_try == 0) ? m_config->get_primary() : m_config->get_secondary();
...
void so_client::fault(const std::string &_log)
{
    ...
    {
        #if defined(HAVE_PA_INTERFACE_H)
        pa::wmi_profiler::add(pa::spam, m_log_host, "spam_check_fault", m_data.m_session_id + "-" + m_envelope->m_id, m_pa_timer.stop()); 
        #endif
        ...
        else if (m_so_try >= g_config.m_so_try)
        {
...
void so_client::success(so_client::spam_status_t _status)
{
    ...
    {
	#if defined(HAVE_PA_INTERFACE_H)
        pa::wmi_profiler::add(pa::spam, m_log_host, "spam_check", m_data.m_session_id + "-" + m_envelope->m_id, m_pa_timer.stop()); 
        #endif
        ...
        else
        {
            ...
            m_envelope->added_headers_.push_back( spam_status::get_headers(_status) );
            
            std::string suid_buffer;
            
            if (spam_status::get_suid_status(m_envelope, suid_buffer))
            {
                m_envelope->added_headers_.push_back(suid_buffer);
            }
