...
void log_response(const boost::asio::streambuf& buf,              
        ...
        time_t session_time)
{
    ...
    session_extracts.push_back(str( boost::format("<< %3% [%1%]:%2%") % buf.size() % ib % (time(0) - session_time)));
...
check::chk_status smtp_client::report_rcpt(bool _success, const std::string &_log, const std::string &_remote)
{
    ...
    #if defined(HAVE_PA_INTERFACE_H)
    pa::wmi_profiler::add(pa::smtp_out, m_relay_name + "[" + m_relay_ip+ "]", m_lmtp ? "lmtp_out_session" : "smtp_out_session", m_data.m_session_id + "-" + m_envelope->m_id, m_pa_timer.stop()); 
    #endif
