...
void black_box_client_rcpt::on_response_read(const std::string &_data, bool _eof)
{
    ...
    if (_eof)
    {
        ...
        {
            report(temp_user_error, m_black_box_parser.m_error_name);
            return;
        }
...
void black_box_client_rcpt::report(const std::string &_response, const std::string &_log, bool success)
{
    ...
    {
	#if defined(HAVE_PA_INTERFACE_H)
        pa::wmi_profiler::add(pa::passport, m_log_host, "blackbox_session", m_check_rcpt.m_session_id + "-" + m_envelope->m_id, m_pa_timer.stop()); 
        #endif
