...
void black_box_client_auth::restart()
{
    ...
    server_parameters::remote_point info = m_connect_count > 1 ? m_switch_cfg->get_secondary() : m_switch_cfg->get_primary();
...
void black_box_client_auth::on_error (const boost::system::error_code& ec, const std::string& logemsg)
{
    ...
    if (m_connect_count == 1)
    {
        m_switch_cfg->fault();
    }
    else if (m_connect_count >= g_config.m_bb_try)
    {
...
{
    m_black_box_parser.parse_buffer(_data, _eof);
    ...
    if (_eof)
    {
        ...
        report(m_black_box_parser.m_auth_success ? check::CHK_ACCEPT : check::CHK_REJECT,
                m_black_box_parser.m_auth_success ? "Success" : m_black_box_parser.m_error_name);
    }
}

void black_box_client_auth::report(const check::chk_status _status, const std::string &_log, bool _success)
{

    if (m_set_status)
    {
	#if defined(HAVE_PA_INTERFACE_H)
        pa::wmi_profiler::add(pa::passport, m_log_host, "blackbox_session", auth_info_.session_id_ + "-AUTH" , m_pa_timer.stop()); 
        #endif
        ...
        g_log.msg(MSG_NORMAL, str(boost::format("%1%-BB-AUTH: login='%2%', status='%3%'") % auth_info_.session_id_  % auth_info_.login_ % _log));
    
        m_io_service.post(boost::bind(m_set_status, _status));
