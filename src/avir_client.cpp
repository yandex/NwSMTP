...
void avir_client::restart()
{
    ...
    server_parameters::remote_point info = (m_try == 0) ? m_config->get_primary() : m_config->get_secondary();
...
void avir_client::fault(const std::string &_log)
{
    ...
    if (m_complete)
    {
        ...
        else if (m_try >= g_config.m_av_try)
        {
            ...
            if (m_complete)
            {
                ...
                pa::wmi_profiler::add(pa::antivirus, m_log_host, "av_check_fault", m_data.m_session_id + "-" + m_envelope->m_id, m_pa_timer.stop()); 
                #endif
...
void avir_client::success(bool _infected)
{
	...
	#if defined(HAVE_PA_INTERFACE_H) 
        pa::wmi_profiler::add(pa::antivirus, m_log_host, "av_check", m_data.m_session_id + "-" + m_envelope->m_id, m_pa_timer.stop()); 
    	#endif
...
void avir_client::handle_timer(const boost::system::error_code& _e)
{
    if (!_e)
    {
        fault("Antivirus connection timeout");
    }   
}
