...
bool smtp_connection::continue_smtp_auth ( const std::string& _cmd, std::ostream &_response )
{
    ...
    switch (auth_.next(_cmd, response))
    {
        ...
        case auth::AUTH_DONE:                                           // got all params start BB Auth
            ...
            auth_.get_session_params(info.login_, info.password_, info.ip_);
...
void smtp_connection::start_passport_auth(const black_box_client_auth::auth_info_t &_info)
{
    ...
    m_bb_check_auth->start( _info, strand_.wrap(bind(&smtp_connection::handle_bb_auth_result, shared_from_this(), _1)) );
}

void smtp_connection::handle_bb_auth_result_helper(check::chk_status _status)
{
    m_proto_state = STATE_HELLO;
...
void smtp_connection::handle_bb_auth_result(check::chk_status _check)
{
    if (!m_bb_check_auth)
        return;
    ...
    handle_bb_auth_result_helper(_check);
}
